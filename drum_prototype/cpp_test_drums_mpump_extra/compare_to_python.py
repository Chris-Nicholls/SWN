...
reference renders in drum_prototype/ (py_closedhat.wav ... py_clap.wav).

Both sides are peak-normalised and compared on onset spectrum only: the
python reference runs at 44.1kHz with its own decay defaults, while the
port runs at 48kHz with musically rescaled decay knobs, so only the
spectral shape of the attack is expected to line up.
"""
import os
import sys
...
PAIRS = [
    ("closedhat", "py_closedhat.wav", "fw_chat_default.wav"),
    ("openhat", "py_openhat.wav", "fw_ohat_default.wav"),
    ("crash", "py_crash.wav", "fw_crash_default.wav"),
    ("ride", "py_ride.wav", "fw_ride_default.wav"),
    ("rimshot", "py_rimshot.wav", "fw_rim_default.wav"),
    ("tom", "py_tom.wav", "fw_tom_default.wav"),
    ("cowbell", "py_cowbell.wav", "fw_cowbell_default.wav"),
    ("clap", "py_clap.wav", "fw_clap_default.wav"),
]
