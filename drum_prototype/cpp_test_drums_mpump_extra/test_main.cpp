/*
 * test_main.cpp — drives the remaining mpump-style voices (closed hat,
 * open hat, crash, ride, rimshot, tom, cowbell, clap) through default,
 * long-decay, low-filter and extreme-"other" settings, writes WAVs into
 * out/, and prints sanity checks (NaN/Inf scan, peak, RMS envelope, ZCR
 * estimate, plus an FFT peak/centroid so the result can be compared
 * against py_*.wav in drum_prototype/).
 *
 * The already-verified mpump kick/snare live in ../cpp_test_drums.
 *
 * Usage:
 *   ./drumtest_mpump_extra [out_dir]
 */
extern "C" {
...
int main(int argc, char **argv)
{
	...
	std::vector<VoiceCase> cases = {
		{"closed_hat", &drum_voice_mpump_closed_hat, {
			{"chat_default",     0.0f, 1.0f,  0.26f, 0.5f, 0.4f},
			{"chat_longdecay",   0.0f, 1.0f,  1.0f,  0.5f, 0.6f},
			{"chat_lowfilter",   0.0f, 0.15f, 0.26f, 0.5f, 0.4f},
			{"chat_dark",        0.0f, 1.0f,  0.26f, 0.0f, 0.4f},
			{"chat_bright",      0.0f, 1.0f,  0.26f, 1.0f, 0.4f},
		}},
		{"open_hat", &drum_voice_mpump_open_hat, {
			{"ohat_default",     0.0f, 1.0f,  0.26f, 0.5f, 0.6f},
			{"ohat_longdecay",   0.0f, 1.0f,  1.0f,  0.5f, 1.5f},
			{"ohat_lowfilter",   0.0f, 0.15f, 0.26f, 0.5f, 0.6f},
			{"ohat_dark",        0.0f, 1.0f,  0.26f, 0.0f, 0.6f},
			{"ohat_bright",      0.0f, 1.0f,  0.26f, 1.0f, 0.6f},
		}},
		{"crash", &drum_voice_mpump_crash, {
			{"crash_default",    0.0f, 1.0f,  0.2f,  0.5f, 1.5f},
			{"crash_longdecay",  0.0f, 1.0f,  1.0f,  0.5f, 4.0f},
			{"crash_lowfilter",  0.0f, 0.15f, 0.2f,  0.5f, 1.5f},
			{"crash_dark",       0.0f, 1.0f,  0.2f,  0.0f, 1.5f},
			{"crash_bright",     0.0f, 1.0f,  0.2f,  1.0f, 1.5f},
		}},
		{"ride", &drum_voice_mpump_ride, {
			{"ride_default",     0.0f, 1.0f,  0.2f,  0.5f, 1.0f},
			{"ride_longdecay",   0.0f, 1.0f,  1.0f,  0.5f, 2.5f},
			{"ride_lowfilter",   0.0f, 0.15f, 0.2f,  0.5f, 1.0f},
			{"ride_dark",        0.0f, 1.0f,  0.2f,  0.0f, 1.0f},
			{"ride_bright",      0.0f, 1.0f,  0.2f,  1.0f, 1.0f},
		}},
		{"rimshot", &drum_voice_mpump_rimshot, {
			{"rim_default",      0.0f, 1.0f,  0.26f, 0.5f, 0.2f},
			{"rim_longdecay",    0.0f, 1.0f,  1.0f,  0.5f, 0.3f},
			{"rim_lowfilter",    0.0f, 0.15f, 0.26f, 0.5f, 0.2f},
			{"rim_low",          0.0f, 1.0f,  0.26f, 0.0f, 0.2f},
			{"rim_high",         0.0f, 1.0f,  0.26f, 1.0f, 0.2f},
		}},
		{"tom", &drum_voice_mpump_tom, {
			{"tom_default",      0.0f, 1.0f,  0.26f, 0.5f, 0.5f},
			{"tom_longdecay",    0.0f, 1.0f,  1.0f,  0.5f, 1.2f},
			{"tom_lowfilter",    0.0f, 0.15f, 0.26f, 0.5f, 0.5f},
			{"tom_floor",        0.0f, 1.0f,  0.26f, 0.0f, 0.5f},
			{"tom_rack",         0.0f, 1.0f,  0.26f, 1.0f, 0.5f},
		}},
		{"cowbell", &drum_voice_mpump_cowbell, {
			{"cowbell_default",   0.0f, 1.0f,  0.26f, 0.5f, 0.4f},
			{"cowbell_longdecay", 0.0f, 1.0f,  1.0f,  0.5f, 0.8f},
			{"cowbell_lowfilter", 0.0f, 0.15f, 0.26f, 0.5f, 0.4f},
			{"cowbell_low",       0.0f, 1.0f,  0.26f, 0.0f, 0.4f},
			{"cowbell_high",      0.0f, 1.0f,  0.26f, 1.0f, 0.4f},
		}},
		{"clap", &drum_voice_mpump_clap, {
			{"clap_default",     0.0f, 1.0f,  0.26f, 0.5f, 0.6f},
			{"clap_longdecay",   0.0f, 1.0f,  1.0f,  0.5f, 1.5f},
			{"clap_lowfilter",   0.0f, 0.15f, 0.26f, 0.5f, 0.6f},
			{"clap_tight",       0.0f, 1.0f,  0.26f, 0.0f, 0.6f},
			{"clap_raw",         0.0f, 1.0f,  0.26f, 1.0f, 0.6f},
		}},
	};
