...
void PhaseDistortionEngineOptimised::Init(BufferAllocator* allocator) {
  Reset();
}

void PhaseDistortionEngineOptimised::Reset() {
  master_phase_ = 0.0f;
  slave_phase_ = 0.0f;
  slave_phase_2_ = 0.0f;
  next_sample_ = 0.0f;
...
void PhaseDistortionEngineOptimised::Render(
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      // Delay modulator by 1 sample (VSO logic)
